...
import wcwidth
import yaml
# from inquirer.themes import GreenPassion # 修改inquirer选择器被选中块的背景�?
...
def save_config(config):
    with open(CONFIG_FILE, 'w', encoding='utf8',) as file:
        # 设置 default_flow_style �? False 以使用块样式输出
        # 设置 encoding �? 'utf-8' 以确保文件以 UTF-8 编码保存
...
def add_server():
    ...
    while True:
        ...
        if name in config:
            print(f"\033[31m服务器名�? '{name}' 已存在，详细信息如下：\033[0m")
    ...
    # IP地址只能是ipv4地址或�?�简单的英文域名,暂时不支持ipv6,因为维护难度�?
    ip = validate_input(
    ...
    port = validate_input("请输入此服务器ssh端口: ",
                          r'^\d+$', "端口号不符合规范，请重新输入�?")
    password = validate_input("请输入服务器的连接密�?: ", r'.+', "密码不能为空，请重新输入�?")
    ...
    save_config(config)
    print(f"\033[32m服务�? : {name} {config[name]} 已添加\033[0m")  # 红色输出
...
def delete_server():
    ...
    confirm = input(f"确认删除服务�? {name} �?? (y/n) Enter默认y: ").strip().lower()
    if confirm == '' or confirm == 'y':
        del config[name]
        save_config(config)
        print(f"\033[32m服务�? {name} 已删除\033[0m")  # 绿色输出
    elif confirm == 'n':
...
def update_server():
    config = load_config()
    # []类型 eg: ['联想小主�?', 'kkltan', 'lenovo', 'tttttt', '百度�?', '腾讯�?']
    servers = list(config.keys())
    if not servers:
        print("没有服务器可以修�?")
        return
    ...
    save_config(config)
    print(f'"{value}"修改成功,当前服务�?"{name1}"的信息为:\n{config[name1]}')
...
def list_servers_1():
    ...
    # answer就是选项的名�?,类型为list,因为还要存储子�?�项的名�?
...
def list_servers_2():
    ...
    for key, value in data.items():
        # 如果选项字符�? �? 服务器子信息 的子�?,说明查找成功, \u3000 即中文的空格
...
def splic_server_info():
    config = load_config()  # 假设 load_config 函数已正确定义并返回�?�?的数据字�?
    result = {}
...
# 罗列每条拼接过的服务信息
# 返回整个服务器的信息的拼�?,包括服务器名,返回类型: list,�?个服务器为一个元�?,含有用于对齐的空格符
# �? : ['    aliyun        lin       nav.sinvon.top       22    ', '    lenovo       sinvon      192.168.1.1         22    ']
def list_servers_details():
    result = []
    data = load_config()
    # 计算各个列的�?大显示宽�?, 修改 max_name_width、max_user_width �? max_ip_width 可以增大减小列的宽度
    ...
    max_port_width = max(len(str(info['port']))
                         for info in data.values())  # 端口通常是数字，不需要�?�虑全角字符
    # 拼接成字符串，�?�虑中文宽度
...
def ssh_connect(server_info):
    ...
    try:
        ...
        if result_code == 255:
            print(
                '\033[31m' + '-------ssh连接失败,请检查服务器信息是否正确后尝试重新连�?--------' + '\033[0m')
...
def upload_file():
    ...
    servers = list(config.keys())
    if not servers:
        print('\033[31m' + '服务器列表为空，请先添加服务�?' + '\033[0m')
        return
    choice = [
        inquirer.List(
            name='server',
            message='上传文件前请先�?�择服务�?,Enter执行',
            choices=servers,
            hints=config,
        ),
    ]
    ...
    # 输入本地文件的路�?,并且判断是否存在
    while True:
        local_folder = input('请输入本地上传的文件的路�?: ')
        # 判断文件是否存在
    ...
    remote_folder = input('请输入保存到远程服务器端的路�?: ')
    ...
    while True:
        ...
        if os.path.exists(local_folder):
            ...
            print('\033[31m' +
                  f'文件 {local_folder} 不存在，请重新输入�??' + '\033[0m')
    local_folder = r"{}".format(local_folder)
    remote_folder = input('请输入从远程服务器下载的文件的路�?: ')
    remote_ip = serverinfo['ip']
    remote_port = serverinfo['port']
    remote_username = serverinfo['user']
    try:
        ...
        # if result_code != 0:
        #     print('\033[31m' + '文件下载失败，请确认服务器或路径信息正确后重新下�?' + '\033[0m')
        # else:
        #     print('\033[32m' + '�?有文件下载完毕！' + '\033[0m')
...
def main():
    while True:
        ...
        if choice == '1':
            ...
            if name:
                confirm = input(
                    f"确认连接服务�? {name} �?? (y/n) Enter默认y: ").strip().lower()
                if confirm == '' or confirm == 'y':
                    print('正在连接到服务器...')
                    # 连接服务�?
            ...
            print(f"\033[32m脚本�?在路�?: {script_dir}\033[0m")
        elif choice == '8':
            print("�?出程�?")
            sys.exit(0)
        else:
